...
# CrediYa Authentication Service - Deploy Script for Podman
# This script manages deployment operations for the authentication service
...
start_services() {
    echo -e "${BLUE}Starting CrediYa Authentication Service...${NC}"
    check_podman_compose
    podman-compose -f $COMPOSE_FILE up -d
    echo -e "${GREEN}Services started successfully${NC}"
    echo -e "${YELLOW}Service URLs:${NC}"
    echo -e "   • Authentication API: ${BLUE}http://localhost:8080${NC}"
...
stop_services() {
    echo -e "${BLUE}Stopping CrediYa Authentication Service...${NC}"
    check_podman_compose
...
restart_services() {
    echo -e "${BLUE}Restarting CrediYa Authentication Service...${NC}"
    stop_services
    start_services
}
